package lzhou.programmingtest.leetcode.test18;
 ...
 * (O(n^3))
 *  * Suppose the indices of the 4 numbers are ```i, j, k, l```
 *  * For ``` i, j ```we iterate through all possible indices. For ``` k, l```, we use two pointer method
 *  * To avoid duplicated results, when we move the pointers, we move them until they point to the next distinct value.
*/
